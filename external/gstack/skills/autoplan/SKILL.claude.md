...
> Curated distribution: updates are managed by `lichao689/skills`. Only the 13 selected skills are installed. References to other slash commands, including `/ship`, are upstream recommendations and are not bundled.
...
_PROACTIVE=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.gstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_SESSION_KIND=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-session-kind 2>/dev/null || echo "interactive")
...
if [ "$_ACTIVATED" = "no" ] && [ "$_SESSION_KIND" != "headless" ]; then
  _FIRST_TASK=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-first-task-detect 2>/dev/null || true)
fi
echo "FIRST_TASK: $_FIRST_TASK"
_LAKE_SEEN=$([ -f ~/.gstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.gstack/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
for _PF in $(find ~/.gstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-telemetry-log" ]; then
      ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${GSTACK_HOME:-$HOME/.gstack}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-timeline-log '{"skill":"autoplan","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get routing_declined 2>/dev/null || echo "false")
...
_CHECKPOINT_MODE=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gstack-*` names. Disk paths stay `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/[skill-name]/SKILL.md`.



Feature discovery, max one prompt per session:
- Missing `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set explain_level terse`.
...
If A: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set telemetry community`
...
If B→A: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set telemetry anonymous`
If B→B: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set telemetry off`
...
If A: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set proactive true`
If B: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set proactive false`
...
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-telemetry-log --event-type first_task_scaffold_shown --skill "TASK_TOKEN" --outcome shown 2>/dev/null || true
...
If B: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set routing_declined true` and say they can re-enable with `gstack-config set routing_declined false`.
...
3. Run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate gstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime} && ./setup --team`"
...
_BRAIN_SYNC_BIN="${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-sync"
_BRAIN_CONFIG_BIN="${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config"
...
```bash
"${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-sync" --discover-new 2>/dev/null || true
"${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-slug 2>/dev/null)"
...
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
**Cross-session decisions.** If `ACTIVE DECISIONS` are listed, treat them as prior settled calls with their rationale — do not silently re-litigate them; if you're about to reverse one, say so explicitly. Reach for `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-decision-search` whenever a question touches a past decision ("what did we decide / why / did we try"). When you or the user make a DURABLE decision (architecture, scope, tool/vendor choice, or a reversal) — NOT a turn-level or trivial choice — log it with `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-decision-log` (`--supersede <id>` for a reversal). Reliable and local; gbrain not required.
...
Curated jargon list lives at `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.
...
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/ETHOS.md`.
...
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-telemetry-log ]; then
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-telemetry-log \
...
Read the `/office-hours` skill file at `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/office-hours/SKILL.md` using the Read tool.
...
SLUG=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
```bash
eval "$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
...
- `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/plan-ceo-review/SKILL.md`
- `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/plan-design-review/SKILL.md` (only if UI scope detected)
- `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/plan-eng-review/SKILL.md`
- `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/plan-devex-review/SKILL.md` (only if DX scope detected)
...
```bash
_TEL=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get codex_reviews 2>/dev/null || echo enabled)
source ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-codex-probe
...
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
