...
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-timeline-log '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
eval "$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG/ceo-plans
```
...
> **STOP.** Before running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed), Read `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/plan-ceo-review/sections/review-sections.md` and execute it
