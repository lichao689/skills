...
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
eval "$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "product"
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-cache get product --project "$SLUG" 2>/dev/null || printf '_(no product digest available yet)_\n'
  printf '\n### %s\n\n' "goals"
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-cache get goals --project "$SLUG" 2>/dev/null || printf '_(no goals digest available yet)_\n'
  printf '\n### %s\n\n' "user-profile"
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-cache get user-profile  2>/dev/null || printf '_(no user-profile digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-cache get recent-decisions --project "$SLUG" 2>/dev/null || printf '_(no recent-decisions digest available yet)_\n'
  printf '\n### %s\n\n' "salience"
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
...
```bash
_CROSS_PROJ=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set cross_project_learnings true`
If B: run `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config set cross_project_learnings false`
...
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/office-hours/sections/design-and-handoff.md` and execute it
...
```bash
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
