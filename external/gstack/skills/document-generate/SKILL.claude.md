...
${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-timeline-log '{"skill":"document-generate","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
REDACT_VIS=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get redact_repo_visibility 2>/dev/null)
[ -z "$REDACT_VIS" ] && REDACT_VIS=$(gh repo view --json visibility -q .visibility 2>/dev/null | tr 'A-Z' 'a-z')
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
