...
For `goal=merge`, apply WAVER's required post-merge local-service refresh. Restart only affected runtime services; docs-only and test-only changes require no runtime restart. For `goal=deploy`, live service, HTTP, job, artifact, and browser evidence outrank stale logs. Delete only owned, merged feature branches and worktrees as required by WAVER policy; preserve shared, integration, or ambiguous branches.
